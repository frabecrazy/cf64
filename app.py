...
def show_main():
    ...
    for device_id in st.session_state.device_list:
        ...
        expanded = st.session_state.device_expanders.get(device_id, True)

        with st.expander(f"🖥️ {base_device}", expanded=expanded):
            ...
            with col1:
                ...
                """, unsafe_allow_html=True)
                shared = st.selectbox("", ["-- Select --", "Personal", "Shared"], index=["-- Select --", "Personal", "Shared"].index(prev["shared"]), key=f"{device_id}_shared")
                ...
                """, unsafe_allow_html=True)
                used = st.selectbox("", ["-- Select --", "New", "Used"], index=["-- Select --", "New", "Used"].index(prev["used"]), key=f"{device_id}_used")
            ...
            with col2:
                ...
                """, unsafe_allow_html=True)
                eol = st.selectbox("", ["-- Select --"] + list(eol_modifier.keys()), index=["-- Select --"] + list(eol_modifier.keys()).index(prev["eol"]) if prev["eol"] in eol_modifier else 0, key=f"{device_id}_eol")
